// ============================================================
// Components — KPIs, charts, table, modal, sidebar
// v1.1 — consolidado y corregido
// Cambios vs v1.0:
//   - FIX: ruta del logo corregida (logo.jpg, sin carpeta assets/)
//   - FIX: parseRanchDate() extraída como helper reutilizable
//   - FIX: GradeLegend() extraído como componente (evita duplicación)
//   - FIX: .pyramid-foot-stat duplicado eliminado de styles.css
//   - FIX: KPI "Última tienta" ahora se calcula desde los datos
//   - MEJORA: currentYear derivado del dato real (no hardcodeado)
// ============================================================
...
/**
 * parseRanchDate — convierte strings de fecha del CSV a año numérico.
 * Formatos soportados: "2023", "15/07/2023", "7-7-25", "15-07-2025"
 * Antes duplicado en HerraderoMini y TientaPage — ahora centralizado.
 */
...
// Pelaje -> color aproximado de la capa
const PELAJE_COLORS = {
...
// Calificaciones de tienta -> color / puntaje
const GRADE_INFO = {
...
// TOPBAR
// ============================================================
function Topbar({ section, search, setSearch, total }) {
  const titles = {
...
function KPIRow({ data, allData, onFilter, currentYear }) {
  ...
  const tcrsHembras = data
    .filter(d => d.S === 'H')
    .map(d => parseInt(d.TCrs, 10))
    .filter(n => !isNaN(n));
  const tcrsAvg = tcrsHembras.length ? (tcrsHembras.reduce((a,b)=>a+b,0) / tcrsHembras.length) : 0;
  const reproductivas = data.filter(d => {
    const a = parseFloat(d.Edad);
    return d.S === 'H' && !isNaN(a) && a >= 3 && a < 14;
  }).length;
    ...
    <div className="kpi-row">
      <div className="kpi kpi-hero">
        ...
        <div className="kpi-value">
          {fmt(total)}<span className="unit">animales</span>
        </div>
        <div className="kpi-foot">
          <span className="mono" style={{color:'#FF330D'}}>●</span> En el hato · {fmt(total)} cabezas
        </div>
      </div>
      {(() => {
      ...
      <div className="kpi">
        ...
        <div className="kpi-value">
          {fmt(nacidosAnio)}<span className="unit">becerros</span>
        </div>
        <div className="kpi-foot">
          {delta >= 0
      ...
      <div className="kpi">
        ...
        <div className="kpi-value">
          {edadAvg.toFixed(1)}<span className="unit">años</span>
        </div>
        <div className="kpi-foot">
          {edades.length} animales con edad registrada
        </div>
      </div>
      <div className="kpi">
        <div className="kpi-label">Crías por madre</div>
        <div className="kpi-value">
          {tcrsAvg.toFixed(1)}<span className="unit">prom.</span>
        </div>
        <div className="kpi-foot">
          {reproductivas} hembras en edad reproductiva
        </div>
      </div>
    </div>
  );
...
// ============================================================
// CHART — Animales nacidos por año
// ============================================================
...
// ============================================================
// CHART — Pelaje (barras horizontales)
// ============================================================
  ...
  return (
    <div className="pyramid">
      <div className="pyramid-head">
        <div className="pyramid-head-side left">
          <span className="pyramid-dot" style={{background:'var(--red)'}}></span>
          Hembras
        </div>
        <div className="pyramid-head-center">Años</div>
        <div className="pyramid-head-side right">
          Machos
          <span className="pyramid-dot" style={{background:'var(--ink)'}}></span>
        </div>
      </div>
...
// ============================================================
// FIX: GradeLegend — antes duplicado en ResumenPage y TientaPage
// Ahora un componente único reutilizable
// ============================================================
      ...
      {grades.map(({ key, name, mods }) => (
        <div key={key} className="gl-item">
          <span className="gl-dot" style={{
            background: GRADE_INFO[key].bg,
            color: GRADE_INFO[key].color,
...
function AnimalTable({ data, onRowClick, sort, setSort, columns }) {
  ...
  if (sorted.length === 0) {
    return <div className="tbl-wrap"><div className="tbl-empty">Ningún animal coincide con los filtros aplicados.</div></div>;
  }

  const headers = {
    arete: '#Reg', nombre: 'Nombre', sex: 'S', year: 'Año',
    edad: 'Edad', pelaje: 'Pelaje', padre: 'Padre', madre: 'Madre', tienta: 'Tienta',
  };
    ...
    <div className="tbl-wrap">
      <table className="tbl">
        <thead>
          <tr>
            {cols.map(c => (
              <th key={c} className="sortable" onClick={() => setSortCol(c)}>
                {headers[c]} {arrow(c)}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {sorted.map((d, i) => (
            <tr key={i} onClick={() => onRowClick(d)} style={{cursor:'pointer'}}>
              ...
              {cols.includes('pelaje') && (
                <td>
                  <span className="pelaje-swatch">
                    <span className="pelaje-dot" style={{background: pelajeColor(d['Pinta/Pelaje'])}}/>
                    {d['Pinta/Pelaje'] || '—'}
                  </span>
                </td>
              )}
...
function AnimalModal({ animal, onClose, allData, onSelectAnimal }) {
  ...
  const idGuess = animal['#Reg'] && animal.AñoN
    ? `${animal['#Reg']} C ${animal.AñoN}`
    : null;
        ...
        <div className="modal-grid">
          <div className="modal-section">
            ...
            <dl className="dl-grid">
              ...
              <dt>Pelaje</dt><dd>
                <span className="pelaje-swatch">
                  <span className="pelaje-dot" style={{background: pelajeColor(animal['Pinta/Pelaje'])}}/>
                  {animal['Pinta/Pelaje'] || '—'}
                </span>
              </dd>
...
// ============================================================
// Expose al namespace global (necesario para el patrón UMD sin bundler)
// ============================================================
Object.assign(window, {
  BrandLogo, Sidebar, Topbar, FilterBar,
  KPIRow, NacimientosChart, PelajeChart, AgePyramid,
  TopSementales, GradeDistribution, GradeLegend,
  AnimalTable, AnimalModal,
  GRADE_INFO, gradeOrder, pelajeColor, PELAJE_COLORS,
  fmt, pct, parseRanchDate,  // FIX: parseRanchDate ahora expuesto globalmente
});
