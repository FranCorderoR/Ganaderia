...
// ── ActualizarBtn ─────────────────────────────────────────────────────────────
function ActualizarBtn({ onSuccess }) {
  const [estado, setEstado] = useState('idle');
  const [msg, setMsg] = useState('Requiere servidor.bat activo');

  const handleClick = () => {
    setEstado('loading');
    setMsg('Conectando con el sistema...');
    fetch('http://localhost:7331/actualizar', {
      method: 'POST',
    ...
    .then(json => {
      ...
      const fechaHora = fecha.replace(/\//g, '.') + ' ' + hora;
      localStorage.setItem('ultima_actualizacion', fechaHora);
      setEstado('ok');
      setMsg(`${json.animales} animales · ${fecha} ${hora}`);
      onSuccess(fechaHora);
      setTimeout(() => location.reload(), 2500);
    })
    .catch(err => {
      setEstado('error');
      if (err.name === 'TimeoutError')           setMsg('Tiempo agotado (2 min)');
      else if (err.message?.includes('fetch'))   setMsg('Servidor no disponible · ejecuta servidor.bat');
      else                                       setMsg(err.message?.slice(0, 55) || 'Error de conexión');
      setTimeout(() => { setEstado('idle'); setMsg('Requiere servidor.bat activo'); }, 8000);
    });
  };

  const estilos = {
    idle:    { color: '#e8a090', background: 'rgba(200,40,26,0.15)',  border: '1px solid rgba(200,40,26,0.35)' },
    loading: { color: '#e8c87a', background: 'rgba(184,137,58,0.2)',  border: '1px solid rgba(184,137,58,0.4)' },
    ok:      { color: '#9ab87a', background: 'rgba(59,74,46,0.35)',   border: '1px solid rgba(107,122,83,0.5)' },
    error:   { color: '#e8a090', background: 'rgba(200,40,26,0.15)',  border: '1px solid rgba(200,40,26,0.35)' },
  };
  const etiquetas = { idle: 'Actualizar datos', loading: 'Actualizando...', ok: 'Actualizado', error: 'Actualizar datos' };
  const msgColor  = estado === 'ok' ? '#9ab87a' : estado === 'error' ? '#ff8070' : 'rgba(230,220,198,0.35)';
    ...
    <div style={{ padding: '14px 16px 0' }}>
      <button
        ...
        style={{
          ...
          padding: '10px 14px', ...estilos[estado],
          borderRadius: 3, fontFamily: 'IBM Plex Sans, sans-serif',
          fontSize: 13, fontWeight: 500, cursor: estado === 'loading' ? 'default' : 'pointer',
      ...
      <div style={{
        marginTop: 7, fontSize: 10, fontFamily: 'JetBrains Mono, monospace',
        color: msgColor, letterSpacing: '0.04em', padding: '0 2px',
      }}>
        {msg}
      </div>
    </div>
...
function Sidebar({ active, onNav, counts, lastUpdated, onUpdateSuccess }) {
  return (
    <aside className="sidebar">
      ...
      <ActualizarBtn onSuccess={onUpdateSuccess} />
  ...
  return (
    <div className="app">
      <Sidebar
        ...
        onUpdateSuccess={setLastUpdated}
      />
      <div className="main">
  ...
  return (
    <>
      <div className="page-head">
        ...
        <div className="meta">Última actualización · {lastUpdated} · {fmt(allData.length)} cabezas</div>
      </div>
